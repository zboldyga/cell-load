...
def make_datamodule(root, train_specs, test_specs, **dm_kwargs):
    ...
    return get_datamodule("PerturbationDataModule", kwargs, batch_size=kwargs.get("batch_size", 16))
...
def test_zero_shot_excludes_celltype(synthetic_data):
    ...
    test_specs  = [ TaskSpec(dataset="dataset1", cell_type=zs_ct, task_type=TaskType.ZEROSHOT) ]

    dm = make_datamodule(root, train_specs, test_specs, few_shot_percent=0.3, embed_key="X_hvg", batch_size=16, control_pert='P0')
    dm.setup()
...
def test_few_shot_includes_celltype_both(synthetic_data):
    ...
    train_specs = [ TaskSpec(dataset="dataset1", task_type=TaskType.TRAINING) ]
    test_specs  = [ TaskSpec(dataset="dataset1", cell_type=fs_ct, task_type=TaskType.FEWSHOT) ]

    dm = make_datamodule(root, train_specs, test_specs, few_shot_percent=0.4, embed_key="X_hvg", batch_size=16, control_pert='P0')
    dm.setup()

    seen_in_train = any(
        subset.dataset.get_cell_type(idx) == fs_ct
        for subset in dm.train_datasets for idx in subset.indices
    )
    seen_in_test  = any(
        subset.dataset.get_cell_type(idx) == fs_ct
        for subset in dm.test_datasets  for idx in subset.indices
    )
...
def test_sampler_groups(synthetic_data):
    ...
    dm = make_datamodule(root, train_specs, [],
                         few_shot_percent=0.3, embed_key="X_hvg",
                         batch_size=3, control_pert="P0")
    dm.setup()
...
def test_collate_fn_shapes_and_keys(synthetic_data):
    ...
    dm = make_datamodule(root, train_specs, [], embed_key="X_hvg", batch_size=4, control_pert='P0')
    dm.setup()
    batch = next(iter(dm.train_dataloader()))

    for key in ("X","basal","pert","pert_name","cell_type","gem_group","gem_group_name"):
        assert key in batch
...
def test_getitem_basal_matches_control(synthetic_data):
    ...
    ds = subset.dataset
    ds.to_subset_dataset("train",
        perturbed_indices=np.array(subset.indices)[
            ds.metadata_cache.pert_codes[subset.indices] != ds.metadata_cache.control_pert_code
        ],
...
def test_to_subset_dataset_control_flag(synthetic_data):
    ...
    ctrl_mask = ds.metadata_cache.pert_codes[all_idxs] == ds.metadata_cache.control_pert_code
