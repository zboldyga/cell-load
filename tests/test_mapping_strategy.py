...
from cell_load.mapping_strategies import RandomMappingStrategy
...
class TestRandomMappingStrategy:
    ...
    def indices_data(self, dataset_with_strategy):
        ...
        return {
            ...
            'cell_type': dataset.get_cell_type(perturbed_indices[0]) if len(perturbed_indices) > 0 else None
        }
    ...
    def test_mapping_creation_at_start_cached(self, indices_data):
        ...
        assert len(mapping_strategy_cached.split_control_mapping["train"]) == len(perturbed_indices) + len(control_indices)
    ...
    def test_no_mapping_regeneration_cached(self, dataloader_cached):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in enumerate(loader_cached):
                ...
                assert cached_strategy.split_control_mapping["train"] == initial_mappings
    ...
    def test_one_to_one_mapping_cached(self, indices_data):
        ...
        for pert_idx, control_indices in mapping_strategy_cached.split_control_mapping["train"].items():
            ...
            for control_idx in control_indices:
                assert control_idx not in cell_type_to_pert_to_control_set[cell_type][pert_name]
    ...
    def test_random_shuffling_on_initialization(self, indices_data):
        ...
        assert strategy1.split_control_mapping["train"] != strategy2.split_control_mapping["train"]
    ...
    def test_get_control_indices_consistency_cached(self, indices_data):
        ...
        pert_idx = perturbed_indices[0]
        result1 = mapping_strategy_cached.get_control_indices(dataset, "train", pert_idx)
        result2 = mapping_strategy_cached.get_control_indices(dataset, "train", pert_idx)
        result3 = mapping_strategy_cached.get_control_indices(dataset, "train", pert_idx)
    ...
    def test_backwards_compatibility_uncached(self, indices_data):
        ...
        for cell_type in cell_types:
            assert len(mapping_strategy_uncached.split_control_pool["train"][cell_type]) >= 0
        
        # Test that get_control_indices works
        pert_idx = perturbed_indices[0]
        results = [mapping_strategy_uncached.get_control_index(dataset, "train", pert_idx) for i in range(5)]
    ...
    def test_multiple_splits_independence(self, indices_data):
        ...
        assert len(mapping_strategy_cached.split_control_mapping["train"]) == len(train_pert_indices) + len(control_indices)
        assert len(mapping_strategy_cached.split_control_mapping["val"]) == len(val_pert_indices) + len(control_indices)
    ...
    def test_cache_parameter_effectiveness(self, indices_data):
        ...
        cell_types = dataset.get_all_cell_types(control_indices)
        for cell_type in cell_types:
            assert len(mapping_strategy_cached.split_control_pool["train"][cell_type]) > 0
            assert len(mapping_strategy_uncached.split_control_pool["train"][cell_type]) > 0 